...
@When("the user enter shipping basic datas as '{name}'")
def step_impl(context, name):
    current_page_class_name = "Pages.WHShippingBasicTab"
    model = input_dynamic_datas(context.table, current_page_class_name)
    context._vp.add_dict(dict_name="wh_shipping_model", value={"{}".format(name): model})


@Then("the shipping '{name}' will be created")
def step_impl(context, name):
    model = context._vp.get("wh_shipping_model")[name]

    # go to the receipt which just created
    Driver.open(context._vp.get("url"))
    Pages.Common.spin_bar.gone()

    # url should be https://fms-stage-qa-5.gofreight.co/warehouse/shipping/entry/{File_No}/
    file_no = Driver.get_url().split("/")[-2]
    model.add(field="file_no", attribute="input", data=file_no)
    model.verify()


@Given("the user has a WH Shipping as '{name}' with only required fields filled")
def step_impl(context, name):
    create_wh_shipping_table = transfer_to_feature_table(
        """
    ...
    # create a new MBL
    Driver.open(gl.URL.WH_NEW_SHIPPING)
    model = input_dynamic_datas(create_wh_shipping_table, "Pages.WHShippingBasicTab", None)
    ...
    model.add(
        field="File No.",
        attribute="input",
        data=Pages.WHShippingBasicTab.file_no_input.get_value(),
    )
    model.add(
        field="Office",
        attribute="autocomplete",
        data=Pages.WHShippingBasicTab.office_autocomplete.get_value(),
    )
